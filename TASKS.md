...
| T163 | Authenticated capabilities, T7 intake cleanup and recovered-run projection | backend/airflow/frontend/infra/QA/docs | load capabilities only after login; product-language scanner card; persistent exact-chip ignore; stale Step4 generation guard; Step5 business-state recovery | BS Docker suites/build pass; 2226 remains absent after rescan; original run displays downloading; fixed network/port retained | done; current batch continues in Step5 |
