...
| T094 | PGT-A resume 临时 BAM 清理与再次恢复 | airflow/backend/qa | T093 | run-local cleanup of `mapping/*.sorted.bam.tmp.*.bam`, cleanup artifact, same-workdir resume | code/tests passed at `0a8e756`; cleanup log deleted 16 stale `G11.sorted.bam.tmp.*.bam` files and remaining tmp count is 0; new resume run `manual__PGTA_20260706_162150_00C4FD__resume__20260707T121252Z` is running with `--cores 64 --rerun-incomplete`, no `--forceall`, and artifacts include `pgta_resume_cleanup`; terminal baseline QC artifacts pending | in_progress |
