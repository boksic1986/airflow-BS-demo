...
| T095 | PGT-A baseline QC Python 库路径与 preflight | airflow/backend/qa/docs | T094 | PGT-A subprocess env sets `LD_LIBRARY_PATH` to conda lib without inheriting container library paths, uses run-local `MPLCONFIGDIR`, writes `logs/pgta.python_preflight.log`, and resumes same workdir | targeted red tests reproduced missing env/preflight; code adds preflight and artifact discovery; full remote tests, Airflow redeploy, and same-workdir resume validation pending | in_progress |
