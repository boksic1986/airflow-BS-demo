...
#update: 20230317, WGS新建新系统，将redis从172.17.61.200:6481改为172.17.61.99:6481
...
sub load_SNV{
	...
	my $redis = RedisDB -> new(host => '172.17.100.38', port => 6382, password => 'BioSan@2024');
...
sub load_CS_SNV{
	...
	my $redis = RedisDB -> new(host => '172.17.100.38', port => 6384, password => 'BioSan@2024');
	$redis -> zadd("SampleList",sub_format_date(localtime(time())),$sampleID);
...
sub load_MT{
	...
	my @subTitle = ('Het/Hom','VAF','Depth','Mom-Het/Hom', 'Mom-VAF', 'Mom-Depth');
	foreach my $subEle (@subTitle) {
