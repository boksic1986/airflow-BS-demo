...
for dirpath, dirnames, filenames in os.walk(os.path.abspath(senddir)):
    for filename in filenames:
        ...
        ossdirSub =  os.path.join("/biosan-bioinfo/ShanDaShengZhi_test/", ossDir) if test else os.path.join("/biosan-bioinfo/ShanDaShengZhi/", ossDir)
...
receiver = config['mail']['receiver_sdsz']
password = config['mail']['password']
smtpserver = config['mail']['smtp_server']

from email import encoders
msgRoot = MIMEMultipart('alternative')
msgRoot['Subject'] = 'WGS数据下载通知-山东山大附属生殖医院有限公司-' + seqDate + '批次'
msgRoot['From'] = sender
msgRoot['To'] = receiver

content='您好：\n\n您送检的全基因组测序样本数据已上传到阿里云oss:/' + ossdirSub + '，请您及时下载！\n\n如有任何问题请及时联系我们，谢谢，祝好！\n\n注意：传输数据自今日起2周后将自动删除，所有数据释放后，请您自行做好备份工作，以免后期数据丢失造成不必要的损失。'
...
att.add_header('Content-Disposition', 'attachment', filename=('gb2312', '', ossDir + '.sampleinfo.山东山大附属生殖医院有限公司.xlsx'))
msgRoot.attach(att)

smtp = smtplib.SMTP()
smtp.connect(smtpserver)
smtp.login(sender, password)
smtp.sendmail(sender, receiver.split(',') , msgRoot.as_string())
smtp.quit()
