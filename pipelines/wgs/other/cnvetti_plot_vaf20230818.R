...
argv = list(
    reference_fai = '/sg2/6.zhangran/WGS_pipeline/WGS_V3/genome/Homo_sapiens_assembly38.fasta.fai',
    samples = 'P33-F,P33-T',
    ...
    vaf_bed = '/tmp/cnv_chrom%s.png',
    out_chrom = '/tmp/cnv_chrom%s.png',
    out_genome = '/tmp/cnv_genome.png',
    cytoband='/sg2/6.zhangran/WGS_pipeline/WGS_V3/annotation/hg38/cytoBand.txt'
);
# Parse command line.
p = arg_parser('CNVetti plotting') %>%
    ...
    add_argument('--bcftools', help='Lower bound of y axis', type="character", default="/bi/software/bcftools-1.18/bcftools") %>%
