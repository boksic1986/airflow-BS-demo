...
def _dispatch_recovery(*, session, run, action, airflow_client):
    ...
    if action.result_status != 'queued':
        ...
        if outcome == 'queued':
            ...
            current = _latest(session, run, action.payload_json['stage'])
...
def request_resume_stage(*, session, settings, airflow_client, analysis_id, attempt, stage, idempotency_key, requested_by):
    ...
    action = _dispatch_recovery(session=session, run=run, action=action, airflow_client=airflow_client)
    data = action.payload_json
