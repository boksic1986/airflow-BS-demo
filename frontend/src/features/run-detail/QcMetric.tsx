...
export function QcMetric({value, judgment}: {value?: string | number | null; judgment?: QcJudgment}) {
  ...
  return <div className="qc-metric">
    {judgment?.unit === "status" && displayed ? <><span className={`qc-value-${status}`}>{text}</span><StatusBadge status={status} /></> : <span className={`qc-value-${status}`}>{text}{judgment?.value != null && judgment.unit ? ` ${judgment.unit}` : ""}</span>}
