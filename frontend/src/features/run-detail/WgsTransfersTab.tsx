...
export function WgsTransfersTab({detail, transfers, refreshKey}: {detail: RunDetail; transfers: WgsTransfer[]; refreshKey?: string | null}) {
  return <div className="transfer-list">{transfers.map((transfer) => (
    <TransferCard detail={detail} transfer={transfer} refreshKey={refreshKey} key={transfer.transfer_id || `${transfer.source}-${transfer.destination}`} />
  ))}{transfers.length === 0 ? <p className="empty-state">No transfers returned.</p> : null}</div>;
}

function TransferCard({detail, transfer, refreshKey}: {detail: RunDetail; transfer: WgsTransfer; refreshKey?: string | null}) {
  ...
  return <section className="transfer-card">
    ...
    {hasDetail ? <>
      ...
      <div className="definition-grid"><div><dt>Progress</dt><dd>{formatPercent(transfer.progress_percent)} / {formatBytes(transfer.bytes_transferred)} of {formatBytes(transfer.bytes_total)}</dd></div><div><dt>Speed</dt><dd>{formatBytes(transfer.speed_bps)}/s</dd></div><div><dt>Files</dt><dd>{transfer.files_completed ?? "-"} / {transfer.files_total ?? "-"}</dd></div>{transfer.transfer_engine ? <div><dt>Engine</dt><dd>{transfer.transfer_engine}</dd></div> : null}<div><dt>Started</dt><dd>{formatDate(transfer.started_at)}</dd></div><div><dt>Finished</dt><dd>{transfer.finished_at || transfer.ended_at ? formatDate(transfer.finished_at || transfer.ended_at) : "-"}</dd></div><div><dt>Last update</dt><dd>{formatDate(transfer.heartbeat_at)}</dd></div></div>
      ...
      {expanded && transfer.transfer_id ? <TransferFiles transferId={transfer.transfer_id} refreshKey={refreshKey} /> : null}
    </> : <div className="transfer-phase-status">
      <p>Stage status is available, but exact byte and file progress has not been recorded.</p>
...
function TransferFiles({transferId, refreshKey}: {transferId: string; refreshKey?: string | null}) {
  ...
  return <div className="transfer-files">
    ...
    <div className="table-wrap"><table className="data-table compact"><thead><tr><th>File</th><th>Status</th><th>Progress</th><th>Speed</th><th>Checksum</th><th>Started</th><th>Finished</th></tr></thead><tbody>
      {items.map((item) => {
        const waitingForCheckpoint = ["accepted", "queued", "pending", "running"].includes(item.status.toLowerCase()) && item.bytes_transferred === 0;
        return <tr key={item.file_key}><td><strong>{item.display_name}</strong>{item.error_message ? <small className="cell-error">{item.error_message}</small> : null}</td><td><StatusBadge status={item.status} /></td><td><div className="file-progress-cell">{waitingForCheckpoint ? <span>Waiting for checkpoint</span> : <><progress max={100} value={item.progress_percent} aria-label={`${item.display_name} progress`} /><span>{formatPercent(item.progress_percent)} · {formatBytes(item.bytes_transferred)} / {formatBytes(item.bytes_total)}</span></>}</div></td><td>{item.speed_bps ? `${formatBytes(item.speed_bps)}/s` : "-"}</td><td><StatusBadge status={item.checksum_status || "pending"} /></td><td>{item.started_at ? formatDate(item.started_at) : "-"}</td><td>{item.ended_at ? formatDate(item.ended_at) : "-"}</td></tr>;
      })}
