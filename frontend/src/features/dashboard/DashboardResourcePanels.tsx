...
function CloudResource({item, heavySlot}: {item?: PlatformResourceSnapshot; heavySlot?: PlatformResourcesResponse["heavy_slot"]}) {
  ...
  return <article className="resource-snapshot"><div className="resource-meter-stack">{!item ? <p>SFS metrics are not available yet. WGS execution is unaffected.</p> : null}<UtilizationBar label="SFS capacity utilization" percent={percent} detail={detail} /><UtilizationBar label="Heavy slots utilization" percent={slotPercent} detail={slotDetail} tone={loadTone(slotPercent)} /><small className="resource-slot-note">{slotNote}</small></div><details className="field-help"><summary>采集详情</summary><p>Reserved heavy work Jobs; one grouped Job consumes one lease. Not individual rules.</p><p>Heavy updated: {formatDate(heavySlot?.updated_at)}</p>{Object.entries(heavySlot?.fields || {}).filter(([, field]) => field?.reason).map(([key, field]) => <p key={key}>{key}: {field?.status} · {field?.reason}</p>)}</details>{item?.error_message ? <p className="inline-error">{item.error_message}</p> : null}</article>;
}
