...
it('restores the run analysis batch instead of a stale submission draft in target confirmation', async()=>{
  window.history.pushState({}, '', '/submit?pipeline=wgs&analysis_id=WGS_SYNTHETIC');
  sessionStorage.setItem('submission-v1:tester:wgs:batch', JSON.stringify('STALE_DRAFT'));
  const detail = {analysis_id:'WGS_SYNTHETIC', pipeline:'wgs', attempt:1, status:'running',
    params:{submission_phase:'execution_review', analysis_batch:'20260910A_CCE_TEST', sequencing_batch:'20260910A'},
    execution_dispatch:{desired_mode:'cce', desired_target:'cce', dispatch_state:'preparing', dispatch_revision:1, allow_switch:true,
      targets:[{target:'cce', mode:'cce', label:'CCE', available:true, status:'available'}, {target:'node-97', mode:'local', label:'Local 97', available:true, status:'available'}]}};
  vi.stubGlobal('fetch', vi.fn((input:RequestInfo|URL)=>{
    const url=String(input);
    const data=url.endsWith('/auth/me')?{username:'tester',role:'operator'}
      :url.endsWith('/platform/capabilities')?{environment:'test',deployed_pipelines:['wgs'],pipelines:[{id:'wgs',display_name:'WGS',dag_id:'bio_wgs',enabled:true,submit_enabled:true,capabilities:['submit'],execution_targets:['cce','local']}]}
      :url.endsWith('/runs/WGS_SYNTHETIC')?detail
      :url.endsWith('/wgs/release')?{source_commit:'cc9bde3',execution_enabled:true,runtime_adapter_enabled:true}
      :{items:[],total:0};
    return Promise.resolve(new Response(JSON.stringify(data),{status:200}));
  }));
  render(<App/>);
  fireEvent.click(await screen.findByRole('button',{name:'Local 97 · available'}));
...
it('keeps inactive catalog options informational and submits no overrides',async()=>{
  ...
  render(<App/>);
  await screen.findByLabelText('Input mode');
  fireEvent.change(screen.getByLabelText('Batch'),{target:{value:'20260912A'}});
