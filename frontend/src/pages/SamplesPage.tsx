...
function Analysis() {
  ...
  return <section className="panel">
    ...
    {!loading ? <div className="table-wrap"><table className="data-table sample-resource-table"><thead><tr><th>Sample / family</th><th>Batch</th><th>Order</th><th>Relation / type</th><th>Project / run</th><th>Status</th></tr></thead><tbody>
      {data.items.map(row => <tr key={JSON.stringify([row.analysis_id, row.execution_id, row.data_id || row.sample_id])}><td><strong>{row.sample_id}</strong>{row.family_id ? <small className="block muted">Family {row.family_id}</small> : null}</td><td>{row.batch_no || "-"}{row.execution_mode ? <small className="block muted">{row.execution_mode === 'sge' ? 'SGE' : 'Local'}</small> : null}</td><td>{row.order_number_masked || "-"}</td><td>{row.family_relation || "-"}<small className="block muted">{[row.sample_type, row.sex].filter(Boolean).join(" / ") || "-"}</small></td><td><Link className="resource-link" to={`/runs/${encodeURIComponent(row.analysis_id)}`}>{row.test_project || row.project_name}</Link><Link className="resource-link secondary mono" to={`/runs/${encodeURIComponent(row.analysis_id)}`}>{row.test_project ? `${row.project_name} · ` : ""}{row.analysis_id}</Link></td><td><StatusBadge status={row.status} />{row.pending_reason || row.status_reason ? <small className="block muted">{row.pending_reason || row.status_reason}</small> : null}</td></tr>)}
