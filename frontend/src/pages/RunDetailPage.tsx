...
export function RunDetailPage() {
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setBundle((current) => ({...current, samples: samples.items, manifest: samples.manifest || []}));
      }
  ...
  }, [analysisId, detail?.status, detail?.step7_cleanup?.latest_action?.status, loadDetail]);
  ...
  async function runStep7Cleanup(batchConfirmation: string) {
    if (!analysisId || !detail?.step7_cleanup?.available) return;
    setActing(true); setActionError(null);
    try {
      await cleanupStep7(analysisId, batchConfirmation);
      await loadDetail();
  ...
  return (
    <div className="page-stack run-detail-page">
      ...
      {detail ? <>
        ...
        {session.hasRole("admin") && (detail.step7_cleanup?.available || detail.step7_cleanup?.latest_action) ? <Step7CleanupPanel capability={detail.step7_cleanup} acting={acting} onCleanup={(batchConfirmation) => void runStep7Cleanup(batchConfirmation)} /> : null}
