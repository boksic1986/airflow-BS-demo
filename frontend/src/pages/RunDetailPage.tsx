...
function WgsSamplesTab({samples, manifest}: {samples: Sample[]; manifest: WgsSampleManifestRow[]}) {
  ...
  return <WgsTable headers={["Sample", "Family / relation", "Received", "Estimated report", "Current stage", "Current Rule", "Rules", "Progress", "Status", "Elapsed", "QC"]} rows={samples.map((sample) => {
    const frozen = manifestBySample.get(sample.sample_id);
    return [sample.sample_id, [sample.family_id || frozen?.family_id, sample.family_relation || frozen?.family_relation].filter(Boolean).join(" / ") || "-", frozen?.received_date || "-", frozen?.estimated_report_date || "-", sample.current_stage || "-", sample.current_rule || "-", `${sample.completed_rules ?? 0}/${sample.total_rules ?? 0}`, formatPercent(sample.progress_percent), <StatusBadge status={sample.status || "unknown"} size="sm" />, sample.elapsed_seconds == null ? "-" : formatSecondsDuration(sample.elapsed_seconds), <StatusBadge status={qcDisplayStatus(sample)} size="sm" />];
