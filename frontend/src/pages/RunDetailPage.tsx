              ...
              {airflowTasks.map((task: AirflowTaskProgress) => (
                <tr key={`${task.task_id}-${task.try_number || "try"}`}>
                  <td>
                    ...
                    {stageDebugLabel(task.task_id) ? <span className="muted block">{task.task_id}</span> : null}
                  </td>
            ...
            <tbody>
              {rules.map((rule) => (
                <tr key={`${rule.rule}-${rule.sample_id || "project"}-${rule.snakemake_jobid || "none"}`}>
                  <td>
                    <strong>{humanStageLabel(rule.rule)}</strong>
                    {stageDebugLabel(rule.rule) ? <span className="muted block">{rule.rule}</span> : null}
                  </td>
...
function LayeredWorkflowTimeline({airflowTasks, rules}: {airflowTasks: AirflowTaskProgress[]; rules: RuleEvent[]}) {
  const pipelineRules = rules.length ? rules : [];
  return (
    <section className="layered-timeline" aria-label="Layered workflow timeline">
      <div className="section-heading">
        <h2>Layered timeline</h2>
        <p>Airflow shows project-level orchestration; Pipeline steps show Snakemake or runner events inside the workflow.</p>
      </div>
      <TimelineLane
        title="Airflow project tasks"
        empty="No Airflow task instances returned yet."
        items={airflowTasks.map((task) => ({
          id: task.task_id,
          label: humanStageLabel(task.task_id),
          status: task.state || "unknown",
          meta: `${task.operator || "operator not set"} · try ${task.try_number ?? "not set"}`,
        }))}
      />
      <TimelineLane
        title="Pipeline steps"
        empty="No rule events captured for this run."
        items={pipelineRules.map((rule) => ({
          id: rule.rule,
          label: humanStageLabel(rule.rule),
          status: rule.status || "unknown",
          meta: rule.sample_id ? `sample ${rule.sample_id}` : "project event",
        }))}
      />
    </section>
  );
...
function TimelineLane({
  ...
  title: string;
  items: Array<{id: string; label: string; status: string; meta: string}>;
  empty: string;
}) {
  return (
      ...
      {items.length ? (
        <ol>
          {items.map((item, index) => (
            <li key={`${item.id}-${index}`} className={`timeline-node timeline-${normalizeStatus(item.status)}`}>
              <span className="timeline-dot">{index + 1}</span>
              <div>
                <strong>{item.label}</strong>
                <small title={item.id}>{item.meta}</small>
              </div>
