...
- `GET /api/platform/capabilities` returns `deployed_pipelines` and only those deployed registry definitions, with their capabilities and execution targets. Registered but undeployed adapters are not exposed as enabled UI choices.
