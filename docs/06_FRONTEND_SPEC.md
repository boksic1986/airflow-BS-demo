...
## T219 stage progress without a numeric denominator
...
- Submit Run uses an optional free-text `Submitted by` ID. The field defaults
  to `jiucheng`, suggests `jiucheng` and `airflow`, accepts custom IDs up to
  128 characters, and remembers the last non-empty value in browser storage.
