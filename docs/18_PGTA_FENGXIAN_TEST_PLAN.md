...
Snakemake 9: /biosoftware/miniconda/envs/snakemake9_env/bin/snakemake 9.23.1
Snakemake 9 logger plugin interface: available
```
...
- 后端触发的 `bio_pgta` 继续保留 Snakemake 8.5.4 metadata v1 闭环，避免破坏已验证路径。
- 新增独立 Airflow-only DAG `bio_pgta_airflow`，使用 `/biosoftware/miniconda/envs/snakemake9_env/bin/snakemake` 9.23.1 和 repo-local logger plugin。
- Snakemake 9 logger plugin 只通过 `PYTHONPATH` 暴露，不修改 `/biosoftware/miniconda/envs/snakemake9_env`，不修改 `/biosoftware/miniconda/envs/snakemake_env`。
...
- `config/pgta_run_config.json` 和 `config.yaml` 可通过 artifacts API 定位；不产生真实 CNV 结果。
...
- 提交受控失败 target：`target=invalid_target`。
- 或提交故意缺失 config 的测试 run。

验收项：

- Airflow DAG failed。
- `analysis_run.error_summary` 包含 dag/run 信息、stderr path 和 stderr 最后 100 行。
...
```text
fengxian run: PGTA_AIRFLOW_20260703_074844
dag_id: bio_pgta_airflow
dag_run_id: manual__PGTA_AIRFLOW_20260703_074844
Airflow state: success
metadata artifact: shared/runs/PGTA_AIRFLOW_20260703_074844/logs/run_metadata.tsv
events artifact: shared/runs/PGTA_AIRFLOW_20260703_074844/logs/events/snakemake_events.jsonl
summary artifact: shared/runs/PGTA_AIRFLOW_20260703_074844/logs/events/snakemake_rule_summary.tsv
XCom summary: event_count=22, failed_jobs=[]
```
