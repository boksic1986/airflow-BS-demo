...
t127_validation_scope: WGS acceptance stops at a successful Snakemake 9 full downstream dry-run for one family; WGS_20260714_180953_9D7981 was intentionally terminated after the 23-job dry-run and must not be described as a completed analysis.
t127_active_run: none. The accepted WGS dry-run record is terminal failed because the host process was deliberately stopped after dry-run evidence; no WGS or NIPT process remains active.
...
t127_images: BS10610 uses backend bs-control-bdecbfb (sha256:73c965a2d1ab...) and frontend bs-control-fd52f5f (sha256:cf13aa210ed6...); archives moved fengxian -> local Windows -> each BS and passed SHA256. BS1069 loaded the same images and remains stopped.
t127_frontend: live browser acceptance shows one NIPT Docker + WGS control plane, zero visible PGT-A labels, zero console errors, JSON-safe /api routing, deployed dashboard aggregation, WGS rule phases, and truthful RSS process-sum labeling when PSS is unavailable.
