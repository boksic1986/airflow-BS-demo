...
## 2026-09-22 download queue display published to BS96
...
passed after RED reproduced Step4 instead of Step5. Published4b09b7b at13:18Z:
releases/20260922-download-waiting-4b09b7b/backend on actual upload-waiting baseline.
Backend only recreated; nginx reloaded;23other container IDs preserved. Gateway
health/tracker/workspace200; C/E/B still upload waiting, D79.4->79.5% running.
No live download queue exists now; queue acceptance is synthetic, not a real-run
claim. Main/production promotion includes this source and publication record.
...
batch-only Overview/QC, reordered sample business fields, exact dropdown
filters (default running), hidden Files/sync details and ledger spacing.
WGS detail-only clinical allowlist is opt-in/authenticated/selected-sample
scoped; no new persistence/global clinical disclosure. No workflow/QC change.
...
is reconciled to this approved main tree while retaining both histories.
...
production change. Custom test execution remains disabled: test root sticky
permission awaits approval, and shared owner source has drifted to68f5dcc with
modified prepare configuration, unlike the audited cc9bde3 contract. Do not
reset or overwrite the shared owner checkout. Other panel work continues.
See task plan/progress ledger and OPT research documents.
...
T255 COMPLETE for image/reference/backend release: user ctapa SSH key resolved final permission gate. Production current20260910-t255-heavy-master-r1, reviewed overlay5bb9e3f; only backend recreated. Live API confirms WGS400ba1c/r2/CCE0.8.3.post1 and truthful quota unavailable. Master SWRdigestf3c197d7...14df458 published/read-back verified; versionedCLI selected, nipttest unchanged. Healthok;907D existing upload retained. DAG/scanner/cloud compute remain paused, NO resumed-cloud acceptance or automatic resume. Older blocker entries below are superseded; exact provenance/rollback in newest HANDOFF.
