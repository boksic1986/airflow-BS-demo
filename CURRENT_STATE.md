...
fix: the runtime gate launches scripts.airflow_handoff from the frozen repository root, clears a failed same-generation sidecar before relaunch, and writes request-scoped aggregate transfer progress with Step1 totals frozen from BATCH_RUNTIME transfer_sources. Step3 now calls the generated status script with --output json. Backend accepts the identity-fenced current sidecar as the terminal projection authority while leaving duplicate terminal timestamps unchanged. The release leaf now raises after releasing leases when upstream tasks failed.
validation: node200 runtime gate passed 12/12 tests; GATK backend submission/evidence tests passed 10/10; the DAG leaf regression passed in the Airflow image; Airflow lists bio_gatk and bio_wgs with no import errors. Public /api/health returned 200 after recreating backend, observer and frontend only; the Airflow worker remained unchanged.
live_checkpoint: prepare, Step1 and Step2 are success. Step1 uploaded 520580463332/520580463332 bytes and 80/80 files. The first Step3 generation failed only because its text output was parsed as JSON; the Master remained running. Airflow was reattached with generation 2 without recreating the Master. The latest checkpoint is 68/522 rules (13 percent), current rule gatk_mark_duplicates, with 68 success and 40 running rule states and healthy logger evidence. The run remains an active test and is not yet Step1-Step6 accepted.
