...
当前目标: T094 进行中；T093 的 64-core resume 因中断遗留 `mapping/G11.sorted.bam.tmp.*.bam` 与 `samtools sort` 冲突失败；T094 已补 run-local tmp cleanup 并重新提交 resume run `manual__PGTA_20260706_162150_00C4FD__resume__20260707T121252Z`，当前仍在 `run_pgta_target`
最近更新时间: 2026-07-07
最后更新 agent: Codex
```
...
last_verified_code_commit: 0a8e756 for T094 PGT-A resume tmp cleanup support
worktree_strategy: single-worktree for now; fengxian is code mirror only
fengxian_mirror: /home/jiucheng/project/airflow-demo cloned from GitHub; runtime validation for T094 ran on origin/codex/airflow/T088-pgta-snakemake-cache at 0a8e756, followed by docs/status evidence update
```
...
| airflow web/api | 12958 | running; `PGTA_20260706_162150_00C4FD` old `--cores 1` DAG run was controlled-interrupted and synced failed; first 64-core resume `manual__PGTA_20260706_162150_00C4FD__resume__20260707T095201Z` failed on stale samtools sort temp BAMs; T094 cleanup resume `manual__PGTA_20260706_162150_00C4FD__resume__20260707T121252Z` is `running` at 2026-07-07 20:13 CST | project image `airflow-demo/airflow:0.1.0`; `/health` returned healthy; Airflow core/UI timezone is `Asia/Shanghai`; T094 resume command uses `--cores 64 --rerun-incomplete` after `--unlock` and `logs/pgta.resume.cleanup.tsv`; terminal QC/artifact validation still pending |
...
- 当前 resume command 已验证为 `--cores 64 --rerun-incomplete` 且不含 `--forceall`；latest DAG run `manual__PGTA_20260706_162150_00C4FD__resume__20260707T121252Z` 仍在 `run_pgta_target`，等待 baseline QC terminal artifacts 后再 sync success/failed
```
