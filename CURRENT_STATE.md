...
```text
release: current -> 20260902-wgs-4.1.1-6c98281-t166-workflow-rule-r1；frontend image airflow-demo/frontend:t166-workflow-rule-r2，image ID sha256:065b1a165bcc...；backend image ID sha256:49635d01a7e4...。
...
validation: BS10610 Docker backend 312 passed；frontend 9 files/35 tests、TypeScript与Vite build通过。代码审查后的阶段fallback、已登记sample边界、analysis.log增量索引和前端死映射回归均通过，最终复审无剩余发现。登录API smoke确认Batch、Finished、六阶段、Rule phase/sample/sequence/message与terminal投影。内置浏览器因内网HTTP URL策略无法执行视觉截图验收，未绕过策略。
