...
validation: isolated BS10610 source passed 17 focused backend tests, 362 complete backend tests, 4 DAG contract tests and actual Airflow 2.9.3 DAG import/callback inspection. The final GATK release merges the deployed 6bed61e handoff/shared-permission line with main and the latency fix; all 14 GATK tests pass. The first complete-suite invocation lacked WGS_STAGE_CONTRACT_PATH and produced two configuration-only failures; rerunning with the repository contract path passed all 362 tests.
deployment: BS10610 current points to releases/20260910-t248-gatk-terminal-b816c91 at code commit b816c91bec1fcfb1013ed80276fb0ea635776c70. Backend, Airflow API, scheduler and worker were recreated; nginx was restarted after backend replacement to refresh its cached Docker address. Root and /api/health return 200. The GATK host runtime uses release 4d6490a with latency-wait=180 and preserves the previously deployed handoff/shared-permission fixes.
reconciliation: retained run GATK_20260909_071908_F45CF7 remains failed without rerun. Evidence-backed progress is restored to 98%, 182/184; cloud_gatk_finalize is failed, all 182 completed rules remain success and all 14 samples are terminal failed. The internal terminal endpoint is idempotent and returns the same projection.
safety: test environment only. No new GATK run, CCE workload, OBS/SFS mutation, WGS run, scanner/auto-dispatch change or BS96 production change occurred. GATK commit 4d6490a exists in the writable BS checkout and deployed release, but two upstream GitLab push attempts were rejected by the configured HTTP credentials; no credential value was logged.
```
...
runtime_fixes: auto_dispatch now follows prepare_sampleinfo -> prepare_analysis before Step1 and preserves approved submission phase through both prepare projections. Stale OBS SDK-only keys are removed from generated 0.8.3 operator config. A prior unbound batch from an earlier attempt is moved intact to attempt history before a retry regenerates it. The 4.2.0 published profile SHA is aligned to 2e7087a6ba646dfb67a940cd77d27ad18fc99e32d92c0ee5243614416f71f38b. The published CCE copy was missing PIPELINE_READY and exposed an ambiguous QC output producer; the marker was restored and WGS_pipe.smk now declares QC_collect_multiqc_qc ahead of QC_SingleQC_merge. Candidate validation built all 363 jobs before the same attempt resumed from Step2.
compatibility: nipttest uses cce-pipeline 0.8.3 and PyMongo 4.9.2 because the production MongoDB 4.0 server supports wire version 7. The Git working source at b067c72 was not modified; the CCE published copy carries hotfix qc-ruleorder-1 with its original WGS_pipe.smk and PIPELINE_READY preserved in T242 evidence for rollback.
...
active_runs: WGS_20260909_193701_95105F-a6, WGS_20260909_193702_44561E-a2, WGS_20260909_193702_10CEB0-a2 and WGS_20260909_193702_FE74F9-a2 remain active. Batch 20260907C is healthy in Step3 with six pre_process_cleanFastq workers and six projected running RuleState rows; the other three are preparing or waiting for the upload lease. A thread heartbeat monitors them every 30 minutes through Step6.
...
safety: no server original FASTQ, project analysis directory, database schema, WGS Git working source, credentials, Docker volume or unrelated workflow was deleted or changed. The 20260907C recovery removed only terminal failed Master Jobs and cleared Step2-Step6 task instances in the same attempt; Step1 was not rerun. An initial scanner activation rediscovered eight obsolete batches; it was stopped immediately, those failed metadata-only attempts were removed from both databases, and the source directories are now explicitly ignored.
```
...
validation: on BS10610, the final selected backend suite passed 28 tests with 53 deselected using the existing cached backend image and network disabled. The complete frontend suite passed 17 files/60 tests and the offline TypeScript/Vite production build passed through the cached T234 builder. Production rebuilt the same source offline and produced airflow-demo/frontend:t240-dashboard-attention-06206f5 at sha256:ce1c0f46d621131e21a7e49670c084a64dce1867300378ae609ab747c0e2cf47 with index-DcuH9NZU.js/index-GaK031bY.css. A fresh backend image build with network disabled could not resolve an uncached FastAPI wheel and was not retried online.
deployment: current points to /data/airflow-WGS/releases/20260909-t240-dashboard-attention-r1 at source 06206f531962ffe8d0d6f5b17cce17883c1fed89. Backend continues to use airflow-demo/backend:t219-stage-terminal-r2 with the T240 source mount; only backend and frontend-nginx were recreated. Live /api/health is OK, dashboard attention returns 7 items, WGS Intake Queue attention returns 2 privacy-safe rows and no longer returns 422.
compose_contract: mainline T240 also contains a later GATK runtime mount whose required production variable is intentionally absent in WGS. The first compose render failed before pointer switch or container restart and the staged env edit was restored. The successful rollout records PRODUCTION_COMPOSE_BASE and retains the already-approved T239 WGS Compose contract; it does not enable or mount GATK runtime state.
safety: automatic analysis remains paused in backend and scanner. Active AnalysisRun and transfer-lease counts were both zero at activation and verification. Scanner schedule/algorithm, observer, Airflow, PostgreSQL, Redis, telemetry, database schema/data, CCE, OBS and SFS were unchanged; non-target container IDs are identical before and after rollout.
```
...
validation: BS10610 cached/offline backend lifecycle tests first passed 20; the production-shaped workspace QC correction passed 77 tests, and the final Run lifecycle QC projector selection passed 91 tests including registry/API coverage. Logger tests passed 8, frontend tests passed 17 files/60 tests, and TypeScript/Vite production build passed. Network and image pulls were disabled.
production: current points to /data/airflow-WGS/releases/20260909-t239-run-lifecycle-qc-r3. Frontend uses airflow-demo/frontend:t239-run-lifecycle-5744738 (sha256:b0930e8e9b058e6edae3fd5bbc011206e0148fb13d98aae71e983fef53eac7e9), serving index-CRdF7Vtc.js and index-DvS8-1p_.css. Public /api/health is 200.
live_projection: 20260906B reports pass in both the Run lifecycle qc_status and Run Detail batch_qc_status, with 9 samples, Workflow operator wgs-scanner, Step7 available with no block reason, and 557 success Rules. Step7 action count remains 0.
...
deployment: BS10610 current points to releases/20260908-t233-intake-capability-r1 with backend/frontend t233-a879e0b. WGS, deployed aggregate and scanner-state return 200; explicit GATK intake returns the intended structured 409. Only backend and frontend were recreated.
...
validation: BS10610 focused backend/runtime suite passed 20 tests; follow-up Step7 generation/config/remnant regressions passed 5, 6 and 6 tests. The .96 cached Node test image passed 5 files/21 tests plus the Step7 retry regression, TypeScript/Vite production build passed, py_compile passed, and the offline PostgreSQL migration chain reached the single 0018 head. No Docker Hub package/image download was used.
deployment: .96 upgraded biodemo from 0017 to 0018 and serves the T227 frontend image on the unchanged 172.17.61.96:12959 boundary. `current` points to `/data/airflow-WGS/releases/20260908-t227-progress-step7-r3`; backend and observer use the T227 source. The frontend proxy was recreated once to refresh backend DNS and health returned 200. Airflow API/scheduler/worker were not restarted and contract v1/WGS 4.1.1 remain active.
recovery: 20260905A Step7 generation 4 completed successfully after generation fencing and frozen-0.8.1 compatibility fixes. 20260902A and 20260825A remain failed/needs recovery because their exact CCE batch-lock ConfigMaps still exist; verified_absent was deliberately not fabricated. 20260904B remains untouched.
continuity: 20260906B had already completed Step1 (18/18) before failing Step2 on the missing node200 runtime root. An exact Airflow dry-run selected submit_step2_master plus 14 downstream tasks and excluded Step1; the same DagRun/attempt was resumed, Step2 and start_step3_monitor succeeded, and Step3 currently has nine running Rule states.
```
...
delivery: implementation commit 0db67bb and the final state-record commit were fast-forwarded into main and synchronized to origin/main and the feature ref. The T222 task worktree was removed after remote verification. Production deployment remains a separate approval.
```
...
```text
source: jiucheng/wgs/T218-stage-terminal-progress-consistency@191fc4c, applied as an isolated semantic delta on current main. Parent-branch transfer-history commits are not replayed.
root_cause: a restricted runtime can publish its exact failed generation after the invoking Airflow task has already exited nonzero. The DAG failure callback and observer shutdown can therefore overtake the status file, leaving the database at accepted or at an older same-attempt failure.
...
```text
scope: clean integration branch from main@ab20035 plus the T216 semantic delta, including accepted node97 capability and terminal submission failure projection. Main's ctapa identity/paths, Step7 route, frontend compatibility and production configuration remain authoritative.
lifecycle: Workflow, Cloud release/Step7, Raw FASTQ backup and Downstream release are independent backend projections. Migration 0017 stores only the latter two registrations with optimistic revision and audit metadata. No backup, copy, validation, scheduling or downstream publication is executed.
...
```text
scope: import only the source currently published by /data/airflow-WGS/current into a clean branch from main; local T213/T214 and other uncommitted development changes are excluded.
...
```text
incident: the first production Step7 maintenance DagRun entered the ordinary prepare and input-transfer path. Its skipped input lease registration returned acquired=false, so acquire_obs_transfer_slot rescheduled until failure while the UI remained queued.
fix: bio_wgs now branches immediately after validation. cleanup_step7 runs only step7_cleanup -> wait_step7_cleanup; the ordinary prepare, approval, OBS lease, Step1-Step6 and finalize paths are skipped. The frontend displays requested/started/finished timestamps, queued/running/success/failed stage text and the exact failure without inventing a percentage, and keeps polling while Step7 is active even when the analysis is already success.
validation: the production-baseline DAG regression failed with the missing branch/tasks, then passed 15/15. The final release build ran the complete frontend suite (15 files, 57 tests) and Vite build successfully. The complete backend suite passed 345 tests with 1 skipped when run with the production `/config` mount. DAG import errors are empty and the public root returns 200.
deployment: current -> /data/airflow-WGS/releases/20260907-wgs-4.1.1-6c98281-t217-step7-route-r1. Airflow api-server/scheduler/worker and frontend were recreated for the route/UI fix; backend was subsequently recreated to load the terminal-action capability fix, and frontend nginx was restarted to refresh backend DNS. Observer, scanner, metrics, PostgreSQL, Redis, volumes and network were preserved. Frontend image is airflow-demo/frontend:t217-step7-route-r1; Airflow image remains bs-control-c706548.
recovery: the exact failed maintenance DagRun was recreated through the Airflow API under the same deterministic ID. The new branch skipped the production path and started only Step7. The action then failed accurately because both the frozen batch root and runtime batch-binding.json were already absent; no manual SFS deletion or fabricated success was performed.
terminal_guard: completed or failed Step7 maintenance actions are now unavailable for repeat submission. The affected action projects `available=false`, `reason=cleanup_failed`, preserving its exact terminal error instead of rendering another unusable cleanup form.
```
...
```text
identity: production SSH execution and node probes now use the dedicated ctapa RSA identity. node200 runtime configuration, kubeconfig, CCE config, OBS configuration and SFS collector inputs live below /home/ctapa; the old hanjj SFS collector is stopped and its key is not mounted by the production stack.
storage: WGS analysis output is rooted at /sg2/50.ctapa/project/HWcloud/WGS_Clinical and the control runtime at /sg2/50.ctapa/project/HWcloud/airflow-wgs/runtime. The PyMongo 4.10.1 compatibility overlay required by the shared nipttest Python was copied to the matching ctapa-owned HWcloud runtime path; the shared environment itself was not modified.
runtime: the real OBS client is /bi/software/obsutil_5.8.3/obsutil. Its progress wrapper retains mode 0700 and the SFS Cloud Eye collector writes the new cloud.json spool every 60 seconds.
ownership: the WGS backend now runs as required WGS_RUNTIME_UID 6801 with shared gid 520, so backend-created runs/<analysis_id>/config paths are owned by ctapa:bioinfo instead of root:bioinfo. The three directories created before this contract were corrected in place; no analysis content was moved or deleted.
...
validation: the full backend suite passes 413 tests, runner/gate suites pass
70 tests, and WGS DAG/deployment contracts pass 34 tests. A gate-only
