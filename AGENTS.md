...
### 数据删除边界（2026-09-15 用户明确约束，优先于历史清理说明）

- 默认清理范围只能是用户当次明确授权的线上资源（例如指定的云端 SFS、OBS 前缀或平台数据库记录），不是对全部线上数据的删除授权。
- 线下/本地项目目录、结果、原始 FASTQ、sampleinfo、pending、运行证据及其副本默认禁止删除，由用户自行处理。远程计算节点、NFS 挂载上的文件仍可属于线下数据；不能因为通过 SSH/API 访问就归为线上。
- `/sg2/50.ctapa/project/HWcloud/WGS_Clinical` 及其他线下项目/结果路径默认受保护。线上与线下批次同名、由 Airflow 创建、已有云端副本，都不构成删除线下数据的授权。
- 只有用户明确要求删除**线下数据**并明确指定批次及删除范围时，才允许删除对应精确路径。若批次存在多个版本/目录或范围不明确，必须先确认。不得扩展到相邻批次或配套数据。
- “清理 WGS”“删除旧数据”“只保留 0910A”“全部重置”不能解释为删除其他线下批次。禁止按通配符/正则匹配后批量删除未逐项授权的线下目录；禁止复用历史一次性清理脚本作为当前授权。
- 执行前在 HANDOFF 记录用户授权、线上/线下分类、逐项目标和保护项；执行后逐项记录实际结果，不只记录数量。快照/备份是否存在必须如实说明，不能假定可恢复。本条不授权任何当前删除操作。
